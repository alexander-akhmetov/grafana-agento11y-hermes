...
.DEFAULT_GOAL := help
.PHONY: help sync format lint test test-all check build clean

help: ## List the targets
	@grep -hE '^[a-z-]+:.*##' $(MAKEFILE_LIST) | sort | awk -F':.*##' '{printf "  \033[36m%-9s\033[0m %s\n", $$1, $$2}'
...
check: lint test ## Everything the CI lint and test jobs run, on one Python
